...
export const InstagromApp = () => {
  ...
  const handleAddComment = async (event: any) => {
    ...
    console.log('Nuevo comentario', event.target);
    startTransition(() =>{

    const element = document.getElementById('post-message') as HTMLInputElement;
    // const [] = useOptimistic
    if(!element) return;

    const newCommentText = element.value.trim();

    if(newCommentText.length===0) return;

     new Promise(resolve=>setTimeout(resolve,3000)).then(response=>{
