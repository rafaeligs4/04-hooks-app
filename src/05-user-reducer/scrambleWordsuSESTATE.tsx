...
export const ScrambleWords = () => {
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if(guess.length<=0 || isGameOver) return;
  ...
  const handleNextWord = ()=>{
    ...
    if(nextLevel >= lengthWords) return;
    
    const nextWordLevel = words[nextLevel];
    console.log(nextWordLevel);
    setScrambledWord(scrambleWord(nextWordLevel));
    setCurrentWord(nextWordLevel);
    setGuess('');
